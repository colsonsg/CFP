...
- **The four home-page hooks are rewritten and approved.** Each now leads with a felt, specific claim rather than a description of a pattern — testimony is used as supporting evidence under each one, never as the headline itself.
- **The youth-decline data point now dates to 2010, not 2015**, added as a third stat in the home page's "What's actually happening" section (tagged `internal analysis`, not `SingStat GHS`, since it's an internal reading of the government data rather than a directly published figure — keep that distinction if you ever edit this).
- **The "isn't a race against a deadline" line is gone from the closing section**, not replaced with equivalent language. Naming the deadline frame to reject it was still putting it in the reader's head; the rest of the section doesn't mention a deadline at all, and that was already doing the job.
- **Every `data-field` span now has real text sitting in the HTML as a default**, not just an empty tag waiting on JavaScript. Previously, anyone viewing source, running a link-preview bot, or browsing with JS disabled saw blank date fields — this fixes that. JS still overwrites these values from `js/config.js` on a normal page load, so editing config.js still works exactly as before; the one tradeoff is that a visitor with JS fully disabled will see whatever date was in the HTML the last time someone updated it, not necessarily this year's — a small edge case, worth knowing about rather than not.
